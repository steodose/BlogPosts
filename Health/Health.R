...
temp <- tempdir()
unzip(zipfile = "/Users/Stephan/Desktop/R Projects/Health/export.zip", exdir = temp)
...
# Create bar chart binned by month
...
records_out_summary %>% 
    ...
    geom_text(aes(label = steps_per_day_label), size = 3, family = "Chivo", 
    ...
    labs(x = "", 
         ...
         subtitle = paste0("Steps taken in 2021, according to Apple Watch data. <span style = 'color:#86F50C';'>**November**</span> was the month I walked most."), 
         caption = "Data: iPhone Health App\nPlot: @steodosescu") +
    theme(plot.title = element_text(face = "bold", size = 16),
          plot.subtitle = element_markdown(),
          legend.position = "none")

ggsave("Step Count Bar Chart.png")

# Add logo to plot
step_count_bar_char_logo <- add_logo(
    plot_path = "/Users/Stephan/Desktop/R Projects/Health/Step Count Bar Chart.png", # url or local file for the plot
...
magick::image_write(step_count_bar_char_logo, "Step Count Chart with Logo.png")


# Create ggridges plot

#filter data to create average line
avg_df_2021 <- records_out %>%
    filter(year == 2021)
...
records_out %>% 
    ...
    labs(x = "Steps", y = "",
         title = "Distribution of Steps in 2021",
    ...
    geom_vline(xintercept = mean(avg_df_2021$steps_adj), color = "red", linetype = "dashed") +
    theme(plot.title = element_text(face="bold")) +
    theme(plot.title = element_text(face="bold"), text = element_text(family = "Chivo"))

ggsave("Step Count Ridge Plot.png")

# Add logo to plot
ridgeplot_logo <- add_logo(
    plot_path = "/Users/Stephan/Desktop/R Projects/Health/Step Count Ridge Plot.png", # url or local file for the plot
    logo_path = "/Users/Stephan/Desktop/R Projects/Health/hex-BTP.png", # url or local file for the logo
    logo_position = "bottom left", # choose a corner
    # 'top left', 'top right', 'bottom left' or 'bottom right'
    logo_scale = 20
...
# save the image and write to working directory
magick::image_write(ridgeplot_logo, "Step Count Ridge Plot.png")


## 2021 vs 2020 line chart
records_20_21 <- records_out %>%
...
records_month %>% 
    ...
    geom_point(size = 2) +
    labs(x = "", y = "Steps",
         title = "Average Steps per Month, 2020 vs. 2021",
    ...
    geom_curve(x = "August", y = 24500,
               ...
               angle = 90,
               arrow = arrow(length = unit(0.25,"cm"))) +
    annotate("text", y = 15000, x = 4, label = "Palm Springs trip\nwith lots of hiking", family = "Chivo", color = "#AF1E2D", vjust = 1, hjust = 0, lineheight = 1) +
    annotate("text", y = 16000, x = "July", label = "Trip to Bandon Dunes\nwhere we walked 18+\nholes for 5 straight days.", family = "Chivo", color = "#AF1E2D", vjust = 1, hjust = 0, lineheight = 1) +
    annotate("text", y = 7500, x = "April", label = "COVID-19 lockdowns\nhit the U.S.", family = "Chivo", color = "#E64B35FF", vjust = 1, hjust = 0, lineheight = 1) +
    scale_color_manual(values = c("#E64B35FF", "#AF1E2D")) + 
    scale_y_continuous(label=comma, limits=c(0,20000)) +
    theme(plot.title = element_text(face="bold")) +
    theme(plot.subtitle = element_markdown())

ggsave("2020 vs 2021 Line Plot.png", height = 6.2, width = 6.5 * asp_ratio, dpi = "retina") 


# Add logo to plot
lineplot_logo <- add_logo(
    plot_path = "/Users/Stephan/Desktop/R Projects/Health/2020 vs 2021 Line Plot.png", # url or local file for the plot
    logo_path = "/Users/Stephan/Desktop/R Projects/Health/hex-BTP.png", # url or local file for the logo
    logo_position = "bottom left", # choose a corner
    # 'top left', 'top right', 'bottom left' or 'bottom right'
    logo_scale = 20
...
# save the image and write to working directory
magick::image_write(lineplot_logo, "2020 vs 2021 Line Plot with Logo.png")


## Create 2020 vs 2021 line chart animation
...
p <- records_month %>% 
    ...
    geom_point(size = 2) +
    labs(x = "", y = "Steps",
         title = "Average Steps per Month, <span style = 'color:#E64B35FF';'>**2020**</span> vs. <span style = 'color:#AF1E2D';'>**2021**</span>",
         subtitle = glue("The latter averaged more steps than the previous year in most months outside of the early part of the year."),
         caption = "Data: iPhone Health App\nGraphic: @steodosescu",
         color = "") +
    theme_custom() +
...
# Save in gif format:
anim_save("2020 vs 2021 Line Plot.gif")


## Create heatmap using gt
...
records_out %>%
    ...
    cols_align(align = "left",
               columns = 1) %>%
    tab_header(title = md("**2021 Step Counts**"),
...
record_2020 <- records_month %>%
    filter(year == 2020) %>% 
    mutate(average = mean(month_steps))

mean(record_2020$average)
